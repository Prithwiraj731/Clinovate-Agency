import React from 'react';
import Container from './Container';
import { Shield, ArrowUp } from 'lucide-react';
import Badge from '../ui/Badge';

export default function Footer({ onNavigate }) {
  const currentYear = new Date().getFullYear();

  const scrollToTop = (e) => {
    e.preventDefault();
    window.scrollTo({ top: 0, behavior: 'smooth' });
  };

  const handleShortcutClick = (e, href) => {
    if (onNavigate) {
      onNavigate('home');
      if (href.startsWith('#')) {
        e.preventDefault();
        const targetId = href.substring(1);
        setTimeout(() => {
          const el = document.getElementById(targetId);
          if (el) {
            el.scrollIntoView({ behavior: 'smooth' });
          }
        }, 100);
      }
    }
  };

  const handlePolicyClick = (e, view) => {
    e.preventDefault();
    if (onNavigate) {
      onNavigate(view);
    }
    setTimeout(() => {
      window.scrollTo({ top: 0, behavior: 'smooth' });
    }, 50);
  };

  return (
    <footer className="bg-[#08080C] border-t border-white/5 pt-16 pb-12 relative overflow-hidden">
      {/* Decorative gradient light blur in footer */}
      <div className="absolute -bottom-48 left-1/2 -translate-x-1/2 w-[600px] h-[300px] rounded-full blur-[150px] bg-accent-gold/5 pointer-events-none" />

      <Container className="relative z-10">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-10 mb-12">
          {/* Brand block */}
          <div className="md:col-span-2">
            <a 
              href="#" 
              onClick={(e) => { e.preventDefault(); handleShortcutClick(e, '#'); }} 
              className="flex items-center gap-2 mb-4"
            >
              <Shield className="text-accent-gold w-6 h-6" />
              <span className="font-serif text-2xl font-bold tracking-tight text-text-primary">
                Clino<span className="text-accent-gold">vate</span>
              </span>
            </a>
            <p className="text-sm text-text-muted max-w-sm mb-6 leading-relaxed">
              We design, build, and secure premium digital solutions for local clinics, cafes, and service businesses.
            </p>
            <div className="inline-flex items-center gap-2 bg-white/5 border border-white/5 py-1.5 px-3 rounded-lg">
              <Shield className="text-accent-emerald w-4 h-4" />
              <span className="text-xs text-text-primary font-mono">Site Shield Active Protection</span>
            </div>
          </div>

          {/* Quick Links */}
          <div>
            <h5 className="font-serif text-sm font-semibold uppercase tracking-wider text-text-primary mb-4">
              Shortcuts
            </h5>
            <ul className="space-y-2.5">
              <li>
                <a 
                  href="#services" 
                  onClick={(e) => handleShortcutClick(e, '#services')}
                  className="text-sm text-text-muted hover:text-accent-gold transition-colors"
                >
                  Our Services
                </a>
              </li>
              <li>
                <a 
                  href="#why-us" 
                  onClick={(e) => handleShortcutClick(e, '#why-us')}
                  className="text-sm text-text-muted hover:text-accent-gold transition-colors"
                >
                  Why Us
                </a>
              </li>
              <li>
                <a 
                  href="#process" 
                  onClick={(e) => handleShortcutClick(e, '#process')}
                  className="text-sm text-text-muted hover:text-accent-gold transition-colors"
                >
                  The Process
                </a>
              </li>
              <li>
                <a 
                  href="#security" 
                  onClick={(e) => handleShortcutClick(e, '#security')}
                  className="text-sm text-text-muted hover:text-accent-gold transition-colors"
                >
                  Site Shield Detail
                </a>
              </li>
            </ul>
          </div>

          {/* Contact / Location */}
          <div>
            <h5 className="font-serif text-sm font-semibold uppercase tracking-wider text-text-primary mb-4">
              Connect
            </h5>
            <p className="text-sm text-text-muted mb-2 font-sans">
              West Bengal, India
            </p>
            <p className="text-sm text-text-muted mb-4 font-sans">
              contact@clinovate.agency
            </p>
            <div className="flex gap-3">
              {/* GitHub Link */}
              <a 
                href="https://github.com/Prithwiraj731" 
                target="_blank" 
                rel="noopener noreferrer" 
                className="p-2 bg-white/5 border border-white/5 rounded-lg text-text-muted hover:text-accent-gold transition-colors"
              >
                <span className="sr-only">GitHub</span>
                <svg className="w-4 h-4 fill-current" viewBox="0 0 24 24"><path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z"/></svg>
              </a>
              {/* LinkedIn Link */}
              <a 
                href="https://www.linkedin.com/in/prithwiraj-mazumdar-963086291/" 
                target="_blank" 
                rel="noopener noreferrer" 
                className="p-2 bg-white/5 border border-white/5 rounded-lg text-text-muted hover:text-accent-gold transition-colors"
              >
                <span className="sr-only">LinkedIn</span>
                <svg className="w-4 h-4 fill-current" viewBox="0 0 24 24"><path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z"/></svg>
              </a>
            </div>
          </div>
        </div>

        {/* Bottom copyright details */}
        <div className="flex flex-col sm:flex-row justify-between items-center border-t border-white/5 pt-8 gap-4">
          <div className="text-xs text-text-muted font-sans text-center sm:text-left">
            <span>&copy; {currentYear} Clinovate. All rights reserved. </span>
            <span className="mx-2">|</span>
            <a 
              href="#" 
              onClick={(e) => handlePolicyClick(e, 'privacy')} 
              className="hover:text-accent-gold transition-colors"
            >
              Privacy Policy
            </a>
            <span className="mx-2">|</span>
            <a 
              href="#" 
              onClick={(e) => handlePolicyClick(e, 'terms')} 
              className="hover:text-accent-gold transition-colors"
            >
              Terms of Service
            </a>
          </div>

          <a 
            href="#" 
            onClick={scrollToTop} 
            className="flex items-center gap-1.5 text-xs font-semibold uppercase tracking-wider text-text-muted hover:text-accent-gold transition-colors duration-200"
          >
            Back to top <ArrowUp className="w-3.5 h-3.5" />
          </a>
        </div>
      </Container>
    </footer>
  );
}
